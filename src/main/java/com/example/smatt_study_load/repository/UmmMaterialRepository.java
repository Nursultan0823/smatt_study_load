...
public interface UmmMaterialRepository extends JpaRepository<UmmMaterial, Integer> {
    ...
    @Query(value = """
            SELECT DISTINCT m.*
            FROM umm_materials m
            WHERE m.discipline_id IN (:disciplineIds)
              AND (CAST(:disciplineId AS INTEGER) IS NULL OR m.discipline_id = :disciplineId)
              AND (CAST(:authorId AS INTEGER) IS NULL OR m.author_id = :authorId)
              AND (
                  CAST(:search AS TEXT) IS NULL OR :search = ''
                  OR m.title ILIKE '%' || :search || '%'
            ...
            ORDER BY m.created_at DESC
            """, nativeQuery = true)
    List<UmmMaterial> searchInDisciplines(
            @Param("disciplineIds") List<Integer> disciplineIds,
            @Param("disciplineId") Integer disciplineId,
            @Param("authorId") Integer authorId,
            @Param("search") String search,
            @Param("materialKind") String materialKind,
    ...
    @Query("""
            ...
            order by d.name
            """)
    List<com.example.smatt_study_load.DTO.UmmDisciplineStatDto> summarizeByDisciplineIds(
            @Param("disciplineIds") List<Integer> disciplineIds);
