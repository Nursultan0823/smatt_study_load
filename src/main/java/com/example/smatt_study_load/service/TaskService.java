...
public class TaskService {
        ...
        if (!hasRole(currentUser, Role.ADMIN)) {
            ...
            if (currentTeacher.getId() != createdById) {
                throw new ResponseStatusException(HttpStatus.FORBIDDEN,
                        "Нельзя создать задание от имени другого преподавателя");
            }
        }
    ...
    private TaskDto toTaskDto(Task task) {
        List<Report> latestReports = getLatestReportsByStudent(task.getReports());
        ...
        dto.setReportsCount(latestReports.size());
        dto.setPendingReportsCount((int) latestReports.stream()
                .filter(report -> report.getStatus() == ReportStatus.SUBMITTED)
                .count());
        dto.setAcceptedReportsCount((int) latestReports.stream()
    ...
    private List<Report> getLatestReportsByStudent(List<Report> reports) {
        if (reports == null || reports.isEmpty()) {
            return List.of();
        }

        Map<Integer, Report> latestByStudent = new HashMap<>();
        for (Report report : reports) {
            if (report.getStudent() == null) {
                continue;
            }

            int studentId = report.getStudent().getId();
            Report currentLatest = latestByStudent.get(studentId);
            if (currentLatest == null || compareReportRecency(report, currentLatest) > 0) {
                latestByStudent.put(studentId, report);
            }
        }
    ...
    private int compareReportRecency(Report first, Report second) {
        LocalDateTime firstSubmittedAt = first.getSubmittedAt();
        LocalDateTime secondSubmittedAt = second.getSubmittedAt();

        if (firstSubmittedAt != null && secondSubmittedAt != null) {
            int submittedAtComparison = firstSubmittedAt.compareTo(secondSubmittedAt);
            if (submittedAtComparison != 0) {
                return submittedAtComparison;
            }
        } else if (firstSubmittedAt != null) {
            return 1;
        } else if (secondSubmittedAt != null) {
            return -1;
        }
    ...
    private List<TaskAnalyticsDto.StudentPerformance> buildTopStudents(List<Report> reports) {
        return reports.stream()
                ...
                .sorted(Comparator
                        ...
                        .reversed()
                        .thenComparing(TaskAnalyticsDto.StudentPerformance::getGradedReports,
    ...
    public void deleteTask(int taskId, Authentication authentication) {
        Task task = taskRepository.findById(taskId)
                .orElseThrow(() -> new RuntimeException("Задача не найдена"));
        authorizeTaskMutation(task, authentication);
        deleteTaskAnnouncements(taskId);
        taskRepository.delete(task);
    }

    private void deleteTaskAnnouncements(int taskId) {
        List<Announcement> taskAnnouncements = announcementRepository.findByTypeInAndTargetId(
                List.of(AnnouncementType.TASK_CREATED, AnnouncementType.TASK_DEADLINE_REMINDER),
                taskId);

        if (taskAnnouncements.isEmpty()) {
            return;
        }

        List<Integer> announcementIds = taskAnnouncements.stream()
                .map(Announcement::getId)
                .toList();
        announcementReadStatusRepository.deleteByAnnouncementIdIn(announcementIds);
        announcementRepository.deleteAll(taskAnnouncements);
    }
